...
pacman::p_load(dplyr, ggplot2, tm, SnowballC, RColorBrewer, wordcloud, glmnet,coefplot,  randomForest, ranger, data.table, stringr )
...
In this lecture through the `Yelp` case study, we will use the `tm` package to transform text into a word frequency matrix. We will build a classifier and conduct sentiment analysis. Finally we build a word cloud to exhibit words for good reviews and bad reviews respectively. 
...
5. Analyses
    1. Build a classifier using your favorite methods: 
        + glmnet
        + glm
    2. Important words related to each rating
        + Word clouds using glm results
    3. Predictions    
    4. LASSO/glm from PCA's 

Appendices:

1. Tips for running a big job
2. Document cleaning
    + Eliminating punctuation
    + Remove the numbers
    + Homogenize the cases
    + Stem the words
    + Remove non-content words (e.g. the, a, ...)
3. Repeat analyses using Random Forest using `randomForest` and `ranger` (skip this at the moment)
4. Repeat analyses using PCA
5. Repeat analyses using term frequency - inverse document frequency (TF-IDF).
...
Data Description: The data here is a small sample (n=100k) of all the reviews (over 1 million) for restaurants in 2014. Unfortunately the old datasets are no longer available. But the Yelp data is still available here that we used a piece in our homework: https://www.yelp.com/dataset
...
+ `TextMing_lasso.RDS`
+ `TextMing_glm.RDS`
+ `comments_to_matrix.jpg`: an image of illustration of term frequency matrix
